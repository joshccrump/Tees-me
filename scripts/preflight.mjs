...
const defaultOutputs = "data/products.json,_data/square_products.json";
let outputPaths = process.env.OUTPUT_PATHS || process.env.OUTPUT_PATH;
if (!outputPaths || String(outputPaths).trim() === "") {
  outputPaths = defaultOutputs;
  console.warn(
    "ℹ️  OUTPUT_PATHS not set; defaulting to 'data/products.json,_data/square_products.json'."
  );
}
outputPaths = String(outputPaths)
  ...
  .join(",");
if (!outputPaths) {
  console.error("❌ OUTPUT_PATHS could not be determined.");
  ok = false;
} else {
  process.env.OUTPUT_PATHS = outputPaths;
  if (!process.env.OUTPUT_PATH) {
    const [first] = outputPaths.split(",");
    if (first) {
      process.env.OUTPUT_PATH = first;
    }
  }
}
...
const optional = {
  ...
  OUTPUT_PATHS: process.env.OUTPUT_PATHS
};
